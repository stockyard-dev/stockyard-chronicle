package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-chronicle/internal/store")
...
func(s *Server)routes(){
    ...
    s.mux.HandleFunc("POST /api/projects/{id}/entries",s.handleCreateEntry)
    s.mux.HandleFunc("DELETE /api/entries/{id}",s.handleDeleteEntry)
    s.mux.HandleFunc("GET /",s.handleUI)
}
func(s *Server)handleHealth(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-chronicle"})}
