...
import (
	...
	"github.com/stockyard-dev/stockyard-chronicle/internal/license"
	"github.com/stockyard-dev/stockyard-chronicle/internal/server"
	"github.com/stockyard-dev/stockyard-chronicle/internal/store"
)

func main() {
	port := getEnv("PORT", "9290")
	dataDir := getEnv("DATA_DIR", "./data")
	licenseKey := os.Getenv("CHRONICLE_LICENSE_KEY")
	...
	log.Printf("Stockyard Chronicle listening on :%s (tier: %s)", port, tier)
